...
const genericTsSchema = {
  parse: (data: unknown): ZodmonValidateResult => ({
    success: true,
    data,
  }),
};
...
type TsSchema<Schema> = {
  ...
  parse: (input: unknown) => ZodmonValidateResult;
};
...
export interface TsTypeProvider
  extends AnyZodmonTypeProvider<{ _schema: unknown }> {
 ...
 */
export const tsTypeProvider: ZodmonRuntimeTypeProvider<TsTypeProvider> = {
