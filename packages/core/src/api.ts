 ...
 */
export function checkApi<const T extends readonly ZodmonEndpointDefinition[]>(
  api: T
) {
  ...
  for (let endpoint of api) {
    ...
    if (paths.has(fullpath)) {
      throw new Error(`Zodmon: Duplicate path '${fullpath}'`);
    }
    paths.add(fullpath);
  }
  ...
  for (let endpoint of api) {
    if (endpoint.alias) {
      if (aliases.has(endpoint.alias)) {
        throw new Error(`Zodmon: Duplicate alias '${endpoint.alias}'`);
      }
  ...
  for (let endpoint of api) {
    if (endpoint.parameters) {
      ...
      if (bodyParams.length > 1) {
        throw new Error(
          `Zodmon: Multiple body parameters in endpoint '${endpoint.path}'`
        );
      }
    }
  }
 ...
 * Mandatory to be used when declaring your endpoint definitions outside zodmon constructor
 * to enable type inferrence and autocompletion
 * @param api - api definitions
 * @returns the api definitions
 */
export function makeApi<const Api extends readonly ZodmonEndpointDefinition[]>(
  api: Api
): Api {
  checkApi(api);
  return api;
...
export function makeParameters<
  const ParameterDescriptions extends readonly ZodmonEndpointParameter[]
...
class ParametersBuilder<T extends ZodmonEndpointParameter[]> {
...
export function makeErrors<
  const ErrorDescription extends readonly ZodmonEndpointError[]
...
export function makeEndpoint<
  const EndpointDescription extends ZodmonEndpointDefinition
>(endpoint: EndpointDescription) {
  return endpoint;
}

export class Builder<const T extends readonly ZodmonEndpointDefinition[]> {
  constructor(private api: T) {}
  addEndpoint<const E extends ZodmonEndpointDefinition>(endpoint: E) {
 ...
 */
export function apiBuilder<const T extends ZodmonEndpointDefinition>(
  endpoint: T
): Builder<[T]> {
...
type MergeApis<
  Apis extends Record<string, readonly ZodmonEndpointDefinition[]>,
...
export function prefixApi<
  ...
  const Api extends readonly ZodmonEndpointDefinition[]
...
export function mergeApis<
  const Apis extends Record<string, readonly ZodmonEndpointDefinition[]>
