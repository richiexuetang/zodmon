...
describe("zodmon hooks", () => {
  describe("keys", () => {
    const apiClient = new Zodmon(api);
    const apiHooks = new ZodmonHooks("test", apiClient);
    beforeAll(() => {});
  ...
  describe("hooks", () => {
    ...
    it("should get id", async () => {
      const apiClient = new Zodmon(`http://localhost:${port}`, api);
      const apiHooks = new ZodmonHooks("test", apiClient);
    ...
    it("should have validation error", async () => {
      ...
      expect(result.current.error).toBeInstanceOf(ZodmonError);
    });
