...
import type { ZodmonEndpointDefinition } from "@zodmon/core";
...
function findPathParam(
  endpoint: ZodmonEndpointDefinition<z.ZodTypeAny>,
  paramName: string
) {
...
function makeOpenApi(options: {
  apis: Array<
    | {
        definitions:
          | readonly ZodmonEndpointDefinition<ZodTypeAny>[]
          | ZodmonEndpointDefinition<ZodTypeAny>[];
      }
    | {
        scheme: string;
  ...
  const openApi: OpenAPIV3.Document = {
    openapi: "3.0.0",
    info: options.info ?? {
      title: "Zodmon : add an info object to 'toOpenApi' options",
      version: "1.0.0",
    },
...
/**
 * Builder to easily create an openapi V3 document from zodmon api definitions
